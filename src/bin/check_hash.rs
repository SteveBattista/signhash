...
fn read_hash(hash_from_file:  & mut [u8], path : &str){
    ...
    let  local_key = HEXUPPER.decode(deserialized_map["HASH"].as_bytes()).unwrap();
    for x in 0..85 {
        hash_from_file[x] = local_key[x];
    }
}
...
fn gethashofile(path: &str, hashalgo: &'static Algorithm) -> Result<(), Box<dyn Error>> {
    ...
    read_hash(& mut hash_from_file, & path.to_owned() + ".sig");
    println!("{} : {}", path, HEXUPPER.encode(digest.as_ref()));
    println!("{}.sig : {}", path, HEXUPPER.encode(hash_from_file.as_ref()));
    Ok(())
}
